...
export default function UsersPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', role: 'user', managerId: '' })
  ...
  const load = useCallback(async () => {
    ...
    if (mRes.ok) {
      ...
      setManagers(data.map((m: { id: string; name: string }) => ({ id: m.id, name: m.name })))
    }
    setLoading(false)
  }, [])
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setShowForm(false); setAdding(false); load()
  }
          ...
          <form onSubmit={handleAdd} className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label className="text-xs font-mono text-[#9494B8] uppercase tracking-wider mb-1.5 block">
                Manager biriktirilsin (oddiy foydalanuvchilar uchun)
              </label>
              <select value={form.managerId} onChange={e => setForm(p => ({ ...p, managerId: e.target.value }))} disabled={adding}
                className="w-full bg-[#111122] border border-[#1E1E35] focus:border-[#FF6B35] text-[#E8E8F5] font-mono text-sm rounded-lg px-3 py-2.5 focus:outline-none transition-colors">
            ...
            {error && (
              <div className="col-span-2 bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-2">
                <p className="text-sm font-mono text-red-400">{error}</p>
              </div>
            )}
            <div className="col-span-2 flex gap-3">
              <button type="button" onClick={() => setShowForm(false)}
                className="px-4 py-2.5 bg-[#161628] text-[#9494B8] text-sm font-display rounded-lg hover:bg-[#1E1E35]">Bekor</button>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#1E1E35]">
                  {['Ism', 'Email', 'Rol', 'Manager', 'Qo\'shilgan', ''].map(h => (
                  ...
                  <tr key={user.id} className="hover:bg-[#111122] transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <select value={user.role} onChange={e => handleRoleChange(user.id, e.target.value)}
                        className="bg-[#111122] border border-[#1E1E35] text-xs font-mono rounded-lg px-2 py-1.5 focus:outline-none focus:border-[#FF6B35] text-[#E8E8F5]">
                        <option value="user">User</option>
                        <option value="admin">Admin</option>
                      </select>
                    </td>
                    <td className="px-5 py-4">
                      <select value={user.managerId || ''} onChange={e => handleManagerChange(user.id, e.target.value)}
                        className="bg-[#111122] border border-[#1E1E35] text-xs font-mono rounded-lg px-2 py-1.5 focus:outline-none focus:border-[#FF6B35] text-[#E8E8F5] max-w-[160px]">
